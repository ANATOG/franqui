...
@section('content')
    <div>
        <div class="row">
            {{ Form::open(array('url' => Config::get('app.url').Config::get('app.admin_directory').'/consultants/edit/'.$item->id, 'name' => 'login-form', 'id' => 'login-form', 'method' => 'put', 'enctype' => 'multipart/form-data')) }}
                ...
                <div class="row">
                    ...
                    <div class="input-field col s12">
                        {{ Form::text('url',(isset($item->url))?$item->url:old('url'), array('id' => 'url', 'placeholder' => 'http://example.com')) }}
                    ...
                    <div class="input-field col s12">
                        @unless(empty($item->image))
                            <a href="{{ url('uploads/'.$item->image) }}" target="_blank"><img src="{{ url('public/uploads/'.$item->image) }}" width="100px"></a>
