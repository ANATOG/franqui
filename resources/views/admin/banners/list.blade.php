...
@section('content')
            ...
            <tbody id="sortable">
                @foreach($items as $item)
                    <tr data-id="{{ $item->id }}">
                        ...
                        <td class="center">
                            @if($item->visible)
                                <a href="{{ Config::get('app.url').Config::get('app.admin_directory').'/banners/visibility/'.$item->id }}"><i class="material-icons">visibility</i></a>
                            @else
                                <a href="{{ Config::get('app.url').Config::get('app.admin_directory').'/banners/visibility/'.$item->id }}"><i class="material-icons">visibility_off</i></a>
                            @endif
                            <a href="{{ Config::get('app.url').Config::get('app.admin_directory').'/banners/edit/'.$item->id }}"><i class="material-icons">mode_edit</i></a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
    ...
    {{ Form::open(array('url' => Config::get('app.url').Config::get('app.admin_directory').'/banners/sort/', 'name' => 'sort-form', 'id' => 'sort-form')) }}
    ...
    <script>
        $(document).ready(function() {
            ...
            $( '#sortable' ).on( 'sortstop', function( event, ui ) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ Config::get('app.url').Config::get('app.admin_directory') }}/banners/sort',
                    dataType: 'json',
