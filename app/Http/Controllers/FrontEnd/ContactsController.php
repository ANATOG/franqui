    ...
    public function sendEmail($data)
    {
        ...
        if ($franchise->count() > 0) {
            ...
            Mail::send('frontend.emails.contacts', $data, function ($message) use ($data, $franchise, $user) {
                ...
                $message->to('consultasfranquicias@franquiciar.com.ar');
                $message->subject('Consulta ' .  strtoupper(html_entity_decode($franchise[0]->name)) . ' | Franquiciar.com.ar');
            });
    ...
    public function sendUserEmail($data)
    {
        ...
        if ($franchise->count() > 0) {
            ...
            Mail::send('frontend.emails.contacts_user', $data, function ($message) use ($data, $franchise, $user) {
                ...
                $message->subject('Consultaste por ' . html_entity_decode($franchise[0]->name) . ' | Franquiciar.com.ar');
            });
        }
    }
