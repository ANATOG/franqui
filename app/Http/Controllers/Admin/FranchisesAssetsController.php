...
class FranchisesAssetsController extends BaseAdminController
{
    ...
    public function updateFranchiseAsset($request, $franchise, $franchisesAssets, $inputs) {
        ...
        if ($request->hasFile('image')) {
        ...
        }elseif ($request['imagedata']) {
            $file = base64_decode(preg_replace('#^data:image/\w+;base64,#i', '', $inputs['imagedata']));
            $folderName = '/uploads/franchises/' . $franchise->id;
            $safeName = md5(microtime().$franchise->name.$franchisesAssets->position).'.'.'jpeg';
            ...
            $franchisesAssets->image = 'franchises/' . $franchise->id . '/' . $safeName;

            if(isset($inputs['old_image'])) {
               if(file_exists(public_path() . '/uploads/' . $inputs['old_image'])){
                   unlink(public_path() . '/uploads/' . $inputs['old_image']);
               }
            }
