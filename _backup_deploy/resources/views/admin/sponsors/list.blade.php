...
@section('search')
    <div class="row search_row">
        <div class="input-field col s12">
            {{ Form::open(array('url' => Config::get('app.url').Config::get('app.admin_directory').'/sponsors/list', 'method' => 'get', 'class'=>'')) }}
...
@section('content')
    <div class="row"><a class="waves-effect waves-light btn blue-grey darken-4" href="{{ Config::get('app.url').Config::get('app.admin_directory').'/sponsors/create' }}">{{ trans('custom.breadcrumbs_create') }}</a></div>
    <div class="row">
    @if(count($items) > 0)
            ...
            <tbody>
                @foreach($items as $item)
                    <tr>
                        ...
                        <td class="center">
                            @if($item->visible)
                                <a href="{{ Config::get('app.url').Config::get('app.admin_directory').'/sponsors/visibility/'.$item->id }}"><i class="material-icons">visibility</i></a>
                            @else
                                <a href="{{ Config::get('app.url').Config::get('app.admin_directory').'/sponsors/visibility/'.$item->id }}"><i class="material-icons">visibility_off</i></a>
                            @endif
                            <a href="{{ Config::get('app.url').Config::get('app.admin_directory').'/sponsors/edit/'.$item->id }}"><i class="material-icons">mode_edit</i></a>

                            <a id="deleteBtn" href="{{ Config::get('app.url').Config::get('app.admin_directory').'/sponsors/delete/'.$item->id }}"><i class="material-icons">delete</i></a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
